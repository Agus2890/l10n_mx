...
class IrAttachmentPaymentMx(models.Model):
    ...
    def sign_file_sf(self,xmldata):
        ...
        wsdl_client = WSDL.SOAPProxy(wsdl_url, namespace)
        zip = False
        params = [user, password, xmldata, zip]
        wsdl_client.soapproxy.config.dumpSOAPOut = 0
        wsdl_client.soapproxy.config.dumpSOAPIn = 0
        wsdl_client.soapproxy.config.debug = 0
        wsdl_client.soapproxy.config.dict_encoding = 'UTF-8'
        ...
        #raise UserError("war",str( resultado ))
        ...
        if codigo_timbrado == '311' or codigo_validacion == '311':
            ...
            uuid=resultado['resultados']['uuid'] or '',
            cfdi_data = {
                ...
                'cfdi_fecha_timbrado': resultado['resultados']['fechaTimbrado'] or False,
                'cfdi_folio_fiscal': resultado['resultados']['uuid'] or '',
                'cfdi_xml':base64.decodestring(resultado['resultados']['cfdiTimbrado'])
            }
            cfdi_xml =base64.decodestring(resultado['resultados']['cfdiTimbrado']) #cfdi_data.pop('cfdi_xml')
            #raise UserError("h",str( cfdi_data ))
            parsedata = xml.dom.minidom.parseString( base64.decodestring(resultado['resultados']['cfdiTimbrado']) )
            ...
            self.payment_ids.write({'cfdi_folio_fiscal': uuid})
            ...
            raise UserError(
                _('Codigo de Error: %s. \n Mensaje: %s.' %
                  (resultado, mensaje))#(resultado['codigo'], mensaje))                
            )

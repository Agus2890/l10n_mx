...
class AmountToText:
    ...
    def _amount_to_text(self, n, lFemenino=0):
        ...
        prim, resto = divmod(n, 10 ** 12)#(n, 10L ** 12)
        if prim != 0:
            if prim == 1:
                cRes = "un billon"
            else:
...
def get_amount_to_text(self, amount, lang, currency=""):
    ...
    if currency.upper() in ('MXP', 'MXN', 'PESOS', 'PESOS MEXICANOS'):
        ...
        sufijo = 'M. E.'
    # return AmountToText(amount, lang, currency)
    amount_text = AmountToText().amount_to_text_cheque(
...
if __name__ == "__main__":
    ...
    letra = AmountToText().amount_to_text_cheque(1500.25, 'pesos', 'M. N.')
    letra = AmountToText().amount_to_text(1500.25)
    # print letra.upper()

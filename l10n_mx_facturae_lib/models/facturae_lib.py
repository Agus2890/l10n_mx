...
try:
   from M2Crypto import RSA
    ...
    logger.error("Package M2Crypto missed")
...
# TODO: Validar que esta instalado openssl & xsltproc
    class facturaeCertificateLibrary(models.Model):
        ...
        def b64str_to_tempfile(self, b64_str="", file_suffix="", file_prefix=""):
            ...
            f = open(fname, 'wb')
            f.write(base64.b64decode(b64_str or ''))
            f.close()
            os.close(fileno)
            return fname
        ...
        def _transform_der_to_pem(self, fname_der, fname_out,
            ...
            if type_der == 'cer':
                ...
                #     fname_der, fname_password_der, fname_out)
        ...
        def _transform_xml(self, fname_xml, fname_xslt, fname_out):
            ...
            """
            cmd = 'xsltproc "%s" "%s" >"%s"' % (
                fname_xslt, fname_xml, fname_out)
            # args = tuple(cmd.split(' '))
            # input, output = exec_command_pipe(*args)
            exec_command_pipe(cmd)
            result = self._read_file_attempts(open(fname_out, "r"))
        ...
        def _get_params(self, fname, params=None, fname_out=None, type='DER'):
            ...
            #     type, fname, cmd_params, fname_out)
            cmd = 'openssl x509 -inform "%s" -in "%s" -noout "%s" -out "%s"' % (
                type, fname, cmd_params, fname_out)
            # args = tuple(cmd.split(' '))
            # # input, output = tools.exec_command_pipe(*args)
            # input, output = exec_command_pipe(*args)
        ...
        def _sign(self, fname, fname_xslt, fname_key, fname_out, encrypt='sha1',
            ...
            #     input.close()
            #     output.close()
            # if 'nomina' in context:
            #     return result    
            keys = RSA.load_key(fname_key)
            # digest = hashlib.new('sha256',str( context.get('cadena_original') )).digest()
            # raise UserError("hola----"+str(context.get('cadena_original')))
            digest = hashlib.new('sha256',str(context.get('cadena_original')).encode('utf-8')).digest()        
            # sello = base64.b64encode(keys.sign(digest, "sha256"))
            sello =str(base64.b64encode(keys.sign(digest, "sha256")),encoding='utf-8')    
            return sello
            #return result
